//input='이용신청서'
export function NFD2NFC(input:string){
const a:(x:string)=>boolean=x=>'ᄀ'<=x&&x<='ᄒ'
const b:(x:string)=>boolean=x=>'ᅡ'<=x&&x<='ᅵ'
const c:(x:string)=>boolean=x=>'ᆨ'<=x&&x<='ᇂ'
let output=''
//12593: ㄱ (일반)
//4352: ᄀ (초성)
//4449: ᅡ
...
export function NFC2NFD (input:string){
    const ko:(x:string)=>boolean = x=>'가'<=x&&x<='힣'
    let output = input.split('').map(v=>{
        if (!ko(v)) return v;
        const a = v.charCodeAt(0) - 44032
        const aa = parseInt(String(a/588))
        const bb = parseInt(String(a/28))%21
        const cc = a%28
