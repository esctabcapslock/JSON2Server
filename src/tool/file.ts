...
import {createWriteStream,WriteStream,rename,unlink,existsSync} from "fs"
import {NFD2NFC} from "./NFC-NFD"

function filiter_filename(file_name:string){
    file_name=NFD2NFC(file_name)
    const file_name_list=file_name.match(/\s|[()]|[+-.]|[0-9]|@|[A-Z]|\[|\]|_|[a-z]|[가-힣]|[ㄱ-ㅎ]|[ᄀ-ᅞ]|[ᅡ-ᇿ]/g) //일부 문자(한/영)만 허용함.
    if(Array.isArray(file_name_list)) file_name=file_name_list.join('');
    else file_name=''
...
export function get_file(req:IncomingMessage,res:ServerResponse, streampath:string='./tmp/', savepath:string='./files/'){ return new Promise((resolve,rejects)=>{


    const method = req.method 
    if (method != 'POST') throw('400 허용되지 않는 방법')

        
    const df:(x:string)=>string = x => x.replace(/\n/g,'\\n+\n').replace(/\r/g,'\\r')
    const ReLU:(x:number)=>number = x => x>0?x:0
    ...
    const 중복파일_목록_알림용:string[] = []; //사용자가 같은 이름을 갖은 파일을 업로드하면, 반영하지 않고 오류라고 알려준다.
            ...
            if (단계==1){
                ...
                while(true){
                    ...
                    if (!file_name && tmp.includes('Content-Disposition:') && tmp.includes('filename=')){
                        ...
                        if(existsSync('./files/'+file_name)||existsSync('./tmp/'+file_name)){
                            중복파일_목록_알림용.push(file_name);
                            file_name=''
                            if(stream) {stream.end(); stream=null}
                        }else{
                            stream = createWriteStream(streampath+file_name,{flags:'wx'})
                        }
                    }
            ...
            if (단계==3){
                ...
                const p1 = new Promise((resolve, reject)=>{
                    let callback = (name=>{return ()=>{
                        ...
                        rename(streampath+name, savepath+name, (err)=>{
                            if(err){
                                console.log(err)
                                resolve('')
                            } 
    ...
    req.on('error',()=>{
        ...
        if(stream) {stream.end(); stream=null}
        if(file_name && existsSync('./tmp/'+file_name)){
            unlink('./tmp/'+file_name,err=>{
